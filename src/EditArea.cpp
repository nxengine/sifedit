...
void EditArea::NextZoomStop(int dir)
{
  ...
  for (curstop = 0; zoomstops[curstop] != -1; curstop++)
    if (zoomstops[curstop] == fZoom)
      break;

  curstop += (dir > 0) ? 1 : -1;

  if (zoomstops[curstop] == -1)
    curstop--;
  if (curstop < 0)
    curstop = 0;
...
void EditArea::Draw(wxDC *dc)
{
  ...
  else if (gd.curmode == EM_VIEW)
  {
    ...
    DrawRect(&bbdc, &sprite->bbox[gd.curdir]);
    ...
    if (!dir->actionpoint.equ(0, 0))
      DrawPoints(&bbdc, &dir->actionpoint, 1);
    if (!dir->actionpoint2.equ(0, 0))
      DrawPoints(&bbdc, &dir->actionpoint2, 1);

    bbdc.SetPen(*wxLIGHT_GREY_PEN);
    if (!sprite->spawn_point.equ(0, 0))
      DrawPoints(&bbdc, &sprite->spawn_point, 1);

    bbdc.SetPen(*wxCYAN_PEN);
    if (!dir->drawpoint.equ(0, 0))
      DrawPoints(&bbdc, &dir->drawpoint, 1);
  }
  else
  {
  ...
  if (fShift.mode)
  {
    ...
    {
      case SM_SIZE:
        DrawGuideText(dc, stprintf("%d x %d", CurSprite()->w, CurSprite()->h));
        break;

      case SM_SHEET_OFFSET:
        DrawGuideText(dc, stprintf("[%d, %d]", CurDir()->sheet_offset.x, CurDir()->sheet_offset.y));
        break;

      case SM_OFFSET_ALL:
        DrawGuideText(dc, stprintf("+[%d, %d]", fShift.totaldx, fShift.totaldy));
        break;
    }
  }
}
...
SIFRect *EditArea::GetCurrentBBox()
{
  ...
  {
    case EM_BBOX:
      return &CurSprite()->bbox[gd.curdir];
    case EM_SOLIDBOX:
...
SIFPoint *EditArea::GetCurrentPointList(int *count_out, SIFPointList **outerstruct)
{
  ...
  *outerstruct = NULL;
  ...
  switch (gd.curmode)
  {
    ...
    case EM_SPAWNPT:
      *count_out = 1;
      return &CurSprite()->spawn_point;

    case EM_ACTIONPT:
      *count_out = 1;
      return &CurDir()->actionpoint;
    case EM_ACTIONPT2:
      *count_out = 1;
      return &CurDir()->actionpoint2;
    case EM_DRAWPT:
      *count_out = 1;
      return &CurDir()->drawpoint;
  }
    ...
    if (fDrag.mouse_moved)
    {
      ...
      if (bbox)
      {
        *bbox = fDrag.draggedrect;
        ...
        if (mainwin->fLinkBBoxCheckbox->GetValue() && gd.curmode == EM_BBOX)
        {
...
void EditArea::DragUpdateAdjustedBBox(int x, int y)
{
  ...
  if (fDrag.mode & DM_ADJUST_X1)
    rect.x1 += distx;
  if (fDrag.mode & DM_ADJUST_X2)
    rect.x2 += distx;
  if (fDrag.mode & DM_ADJUST_Y1)
    rect.y1 += disty;
  if (fDrag.mode & DM_ADJUST_Y2)
    rect.y2 += disty;
...
int EditArea::SelectDragMode(int px, int py)
{
  ...
  if (py >= y1 && py <= y2)
  {
    if ((nc = abs(px - x1)) < xclosest)
    {
      xclosest = nc;
      xwinner  = DM_ADJUST_X1;
    }
    if ((nc = abs(px - x2)) < xclosest)
    {
      xclosest = nc;
      xwinner  = DM_ADJUST_X2;
    }
  }

  if (px >= x1 && px <= x2)
  {
    if ((nc = abs(py - y1)) < yclosest)
    {
      yclosest = nc;
      ywinner  = DM_ADJUST_Y1;
    }
    if ((nc = abs(py - y2)) < yclosest)
    {
      yclosest = nc;
      ywinner  = DM_ADJUST_Y2;
    }
  }
...
#define CLOSE_DIST 5
  ...
  if (xclosest <= CLOSE_DIST)
    result |= xwinner;
  if (yclosest <= CLOSE_DIST)
    result |= ywinner;
...
void EditArea::UpdateCtrlShiftStates()
{
  ...
  if (fShift.mode != oldShiftMode)
  {
    if (fShift.mode)
    {
      ...
      fShift.expand_bbox     = CurSprite()->bbox[gd.curdir].equ(0, 0, CurSprite()->w - 1, CurSprite()->h - 1);
  ...
  if (deltax)
  {
    switch (fShift.mode)
    {
      ...
      case SM_SHEET_OFFSET:
      {
        ...
        if (dir->sheet_offset.x < 0)
          dir->sheet_offset.x = 0;
      }
      break;
  ...
  if (deltay)
  {
    switch (fShift.mode)
    {
      ...
      case SM_SHEET_OFFSET:
      {
        ...
        if (dir->sheet_offset.y < 0)
          dir->sheet_offset.y = 0;
      }
      break;
  ...
  if (deltax || deltay)
  {
    if (fShift.mode == SM_SIZE)
    {
      if (fShift.expand_bbox)
        CurSprite()->bbox[gd.curdir].set(0, 0, CurSprite()->w - 1, CurSprite()->h - 1);
...
void EditArea::UpdateShiftStatusText()
{
  ...
  {
    case SM_SHEET_OFFSET:
      status = stprintf("Adjusting sheet offset: [%d, %d]", CurDir()->sheet_offset.x, CurDir()->sheet_offset.y);
      break;
    ...
    case SM_SIZE:
      status = stprintf("Sizing current sprite: %dx%d", CurSprite()->w, CurSprite()->h);
      break;
  }
...
{
  if (sheet_x != other.sheet_x)
    return false;
  if (sheet_y != other.sheet_y)
    return false;
  if (w != other.w)
    return false;
  if (h != other.h)
    return false;
  if (sheetno != other.sheetno)
    return false;
  if (zoom != other.zoom)
    return false;
