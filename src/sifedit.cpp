...
bool MyApp::OnInit()
{
  ...
  strcpy(gd.openfilename, "./data/sprites.sif");
...
bool load_sif(const char *fname)
{
  ...
  for (int s = 0; s < nsprites; s++)
  {
    ...
    spr->name = s < names.size() ? names.at(s) : "NameUnknown";
    spr->comment = s < comments.size() ? comments.at(s) : "";

    spritelist.AddSprite(spr);
  }

  LoadGroupsSection(&sif);
  if (gd.groupnames.size() == 0)
  {
    stat("load_sif: SIF directory section is empty or missing, adding initial '/' grouping");
    gd.groupnames.push_back("/");
  }
...
bool LoadSessionSection(SIFLoader *sif)
{
  ...
  // sanity checking
  if (!IsValidSprite(gd.cursprite))
    gd.cursprite = 0;
  if (!IsValidFrame(gd.cursprite, gd.curframe))
    gd.curframe = 0;
  if (!IsValidDir(gd.cursprite, gd.curdir))
    gd.curdir = 0;
...
bool save_sif(const char *fname)
{
  ...
  if (SaveStringSection(&sif, SIF_SECTION_SHEETS, &gd.sheetfiles))
    return 1;
  ...
  for (i = 0; i < nsprites; i++)
  {
    ...
    names.push_back(sprite->Name() ? sprite->Name() : "");
    comments.push_back(sprite->Comment() ? sprite->Comment() : "");
  }
  ...
  if (SaveStringSection(&sif, SIF_SECTION_NAMES, &names))
    return 1;
  if (SaveStringSection(&sif, SIF_SECTION_COMMENTS, &comments))
    return 1;
...
bool SaveGroupsSection(SIFLoader *sif)
{
  ...
  int ngroups = gd.groupnames.size();

  // first comes the array of group names
  data.Append8(ngroups);
  for (i = 0; i < ngroups; i++)
  {
    SIFStringArraySect::WritePascalString(gd.groupnames.at(i), &data);
  }
...
{
  std::string str(pathPrefix);
  str += std::string(path);
  return stprintf("%s", str.c_str());
}
...
void ReorderGroups(std::vector<int> *movelist, int insertafter)
{
  ...
  for (int i = 0; i < gd.groupnames.size(); i++)
  {
    if (!f(*movelist,i))
    {
      newgroups.push_back(gd.groupnames.at(i));
      old_to_new[i] = gi++;
    }
  ...
  for (int i = 0;; i++)
  {
    ...
    if (rec->group >= 0 && rec->group <= gd.groupnames.size())
...
void DeleteGroup(int delgroup, int kickgroup)
{
  ...
  gd.groupnames.erase(gd.groupnames.begin()+delgroup);
}
...
{
  gd.sheetfiles.erase(gd.sheetfiles.begin()+sheetno);
...
bool load_spritesheet(wxImage *image, const char *filename)
{
  ...
  if (!image->LoadFile(wxString(filename, wxConvUTF8)))
    return 1;
  if (!image->IsOk())
    return 1;
  return 0;
...
SIFDir *CurDir()
{
  ...
  if (gd.curframe >= 0 && gd.curframe < spr->nframes && gd.curdir >= 0 && gd.curdir < spr->ndirs
