...
{
  if (fSheetNo == sheetno && fSpriteSheet)
    return 0;
  fSheetNo = -1;
  ...
  const char *filename = gd.sheetfiles.at(sheetno).c_str();
  if (!filename)
  {
...
wxImage *SheetManager::GetSubImage(int x, int y, int w, int h)
{
  ...
  if (src_x2 >= fSpriteSheet->GetWidth())
    src_x2 = (fSpriteSheet->GetWidth() - 1);
  if (src_y2 >= fSpriteSheet->GetHeight())
    src_y2 = (fSpriteSheet->GetHeight() - 1);
  if (src_x1 >= fSpriteSheet->GetWidth())
    nodraw = true;
  if (src_y1 >= fSpriteSheet->GetHeight())
    nodraw = true;
  if (src_x2 < 0)
    nodraw = true;
  if (src_y2 < 0)
...
wxImage *SheetManager::GetSprite(int s, int f, int d)
{
  ...
  return GetSubImage(sprite->frame[f].dir[d].sheet_offset.x, sprite->frame[f].dir[d].sheet_offset.y, sprite->w,
                     sprite->h);
}
